...
public class TreeViewer extends JPanel {
	...
	public TreeViewer() {
		// Create RobotModel and populate it with robots.
		RobotModel model = new RobotModel(new Dimension(500, 500));
		
		CarrierRobot top = new CarrierRobot(10, 10, 2, 2, 200, 250);
		CarrierRobot mid = new CarrierRobot(10, 10, 1, 3, 150, 100);
		
		model.add(mid, top);
		model.add(new WheeledRobot(10, 10, 3, 2, 70, 40), top);
		model.add(new WheeledRobot(0, 0, 1, 1, 80, 35), mid);
		model.add(top, model.root());
		model.add(new WheeledRobot(0, 0, 2, 3), model.root());
		model.add(new WheeledRobot(0, 0, 1, 1), model.root());
		model.add(new WheeledRobot(0, 0, -1, -1), model.root());
		model.add(new WheeledRobot(0, 0, 5, 5), model.root());
		
		// Create an instance of Task2 and set its adaptee to the RobotModel.
