 ...
 * 
 * For the Robot application, for example, a FormComponent might contain two
 * FormElementsComponents: a RobotFormElementComponent and a 
 * ColourFormElementComponent. The RobotFormElementComponent would display 
 * fields common to all Robots (location, width, height etc.), while the 
 * ColourFormElementComponent would allow the user to choose a colour for 
 * painting the Robot. The combination of the two FormElements would allow a 
 * user to enter sufficient information for describing a Robot with colour.
 * 
 * The FormComponent's FormHandler is responsible for processing FormElement
 * field data. In the Robot application, FormHandler implementations create
 * different kinds of Robots based on form data.
 * 
 * @see robot.forms.util.Form
